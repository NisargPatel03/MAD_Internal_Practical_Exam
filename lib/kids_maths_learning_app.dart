...
//   flutter_tts: ^3.8.3
// Optional: audioplayers for sound effects, vibration for haptics
...
class _HomeScreenState extends State<HomeScreen> with TickerProviderStateMixin {
                ...
                children: [
                  ElevatedButton.icon(
                    ...
                    style: ElevatedButton.styleFrom(
                      backgroundColor:
                          _mode == Mode.practice ? Colors.teal : Colors.grey[300],
                      foregroundColor:
                  ...
                  ElevatedButton.icon(
                    ...
                    style: ElevatedButton.styleFrom(
                      backgroundColor: _mode == Mode.quiz ? Colors.teal : Colors.grey[300],
                      foregroundColor: _mode == Mode.quiz ? Colors.white : Colors.black,
                    ),
                  ),
                ],
              ),
  ...
  Widget _iconForOp(Operation op) {
    switch (op) {
      ...
      case Operation.division:
        return const Icon(Icons.percent, color: Colors.purple);
    }
  }
}
...
class _PracticeScreenState extends State<PracticeScreen> with SingleTickerProviderStateMixin {
  ...
  void _generate() {
    ...
    // For division, ensure divisible numbers for clean results
    if (widget.operation == Operation.division) {
      b = _rand.nextInt(range) + 1; // avoid zero
  ...
  num _correctAnswer() {
    switch (widget.operation) {
      ...
      case Operation.multiplication:
        return a * b;
      case Operation.division:
        return a ~/ b; // integer division (we ensured divisible)
    }
  }
  ...
  void _submit() {
    ...
    if (ans == correct) {
      ...
      feedbackColor = Colors.green[300]!;
      widget.tts.speak('Great! Correct');
      // small celebration: new problem after delay
      Future.delayed(const Duration(milliseconds: 600), _generate);
    } else {
      feedback = 'Try again';
      feedbackColor = Colors.red[300]!;
